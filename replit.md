...
- 3D viewer (`components/model-viewer.tsx`) clones GLTF scene + materials per instance, disposes cloned materials on unmount/url change to avoid GPU leaks, walks click events up to the nearest named ancestor, and applies emissive highlight: orange = selected, sky-blue (`#38bdf8`, brighter) = preview (clicked but not yet tagged), blue (`#3b82f6`, dim) = tagged-but-not-selected.
